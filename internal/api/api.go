...
func newHandler(q *pgstore.Queries) http.Handler{
		...
		// Rota para subscribe do websockets 
...
func (h apiHandler) handleSubscribe(w http.ResponseWriter, r *http.Request) {
	...
	slog.Info("new cliente connected", "room_id", rawRoomID, r.RemoteAddr)
...
func (h apiHandler) notifyClients(msg Message) {
	...
	for conn, cancel := range subscribers {
		if err := conn.WriteJSON(msg); err != nil {
			slog.Error("falied to send message to client", "error", err)
			cancel()
		}
	}
}
func (h apiHandler) handleCreateRoom(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		slog.Error("failed to insert rooom", "error", err)
...
func (h apiHandler) handleCreateRoomMessage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "something went wronh", http.StatusInternalServerError)
		return
	}
